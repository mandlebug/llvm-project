...
// RUN: %clang_cc1 -fopenmp -x c -triple %itanium_abi_triple -include-pch %t -Wno-tentative-definitions -verify %s -emit-llvm -o - -femit-all-decls -disable-llvm-passes | FileCheck --check-prefix=CHECK-LOAD %s
...
// RUN: %clang_cc1 -fopenmp -x c -triple %itanium_abi_triple -include-pch %t -Wno-tentative-definitions -verify %s -emit-llvm -o - -femit-all-decls -disable-llvm-passes -fopenmp-version=45 | FileCheck --check-prefixes=CHECK-LOAD,OMP45-LOAD %s
...
// RUN: %clang_cc1 -fopenmp-simd -x c -triple %itanium_abi_triple -include-pch %t -Wno-tentative-definitions -verify %s -emit-llvm -o - -femit-all-decls -disable-llvm-passes | FileCheck --check-prefix SIMD-ONLY0 %s
